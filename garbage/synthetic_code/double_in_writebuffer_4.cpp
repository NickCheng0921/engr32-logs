    ...
    void process_thread() {
        ...
        while (1) {
            if (!fifo1.empty() && !fifo2.empty()) {
                ...
                ac_int<64, false> lower = d1.data.slc<64>(0);
                ac_int<64, false> upper = d2.data.slc<64>(64);

                ac_int<128, false> combined;
                combined.set_slc(0, lower);
                combined.set_slc(64, upper);

                ac_int<128, false> final_data = combined ^ 0xA5A5A5A5;
