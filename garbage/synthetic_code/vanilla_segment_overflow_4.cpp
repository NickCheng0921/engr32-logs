    ...
    void segment_thread() {
        ...
        while (1) {
            ...
            if (data_i.PopNB(input)) {
                Struct_Out output;
                ac_int<128, false> lo = input.data.slc<128>(0);
                ac_int<128, false> hi = input.data.slc<128>(128);
                ac_int<129, false> result = (ac_int<129, false>)lo + hi; // to check for overflow, we extend lo

                output.status[0] = result[128];          // overflow
