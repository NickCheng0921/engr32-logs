    ...
    void process_thread() {
        ...
        while (1) {
            if (!fifo1.empty() && !fifo2.empty()) {
                ...
                ac_int<128, false> lower = d1.data.slc<128>(0);
                ac_int<128, false> upper = d2.data.slc<128>(128);

                ac_int<256, false> combined;
                combined.set_slc(0, lower);
                combined.set_slc(128, upper);

                ac_int<256, false> final_data = combined ^ 0xDEADBEEF;
