    ...
    void process_thread() {
        ...
        while (1) {
            if (!fifo1.empty() && !fifo2.empty()) {
                ...
                ac_int<16, false> lower = d1.data.slc<16>(0);
                ac_int<16, false> upper = d2.data.slc<16>(16);

                ac_int<32, false> combined;
                combined.set_slc(0, lower);
                combined.set_slc(16, upper);

                ac_int<32, false> final_data = combined ^ 0xDEADBEEF;
