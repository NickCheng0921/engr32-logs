    ...
    void process_thread() {
        ...
        while (1) {
            if (!fifo1.empty() && !fifo2.empty()) {
                ...
                ac_int<32, false> lower = d1.data.slc<32>(0);
                ac_int<32, false> upper = d2.data.slc<32>(32);

                ac_int<64, false> combined;
                combined.set_slc(0, lower);
                combined.set_slc(32, upper);

                ac_int<64, false> final_data = combined ^ 0xDEADBEEF;
