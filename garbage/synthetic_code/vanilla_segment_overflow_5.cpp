    ...
    void segment_thread() {
        ...
        while (1) {
            ...
            if (data_i.PopNB(input)) {
                Struct_Out output;
                ac_int<256, false> lo = input.data.slc<256>(0);
                ac_int<256, false> hi = input.data.slc<256>(256);
                ac_int<257, false> result = (ac_int<257, false>)lo + hi; // to check for overflow, we extend lo

                output.status[0] = result[256];          // overflow
